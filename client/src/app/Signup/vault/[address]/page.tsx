...
const page = ({ params }: { params: { address: string } }) => {
  ...
  return (
    <div className='grid grid-cols-2 border-2 border-input rounded-xl w-fit p-8 mx-auto my-auto mt-[10vh] shadow-lg'>
      <div className="">
      <svg className='scale-2' width="34" height="32" viewBox="0 0 34 32" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <h1 className='font-medium text-[24px] mt-2'>Signup</h1>
      </div>
      <form onSubmit={createUser} className='min-w-[400px] justify-self-end'>
        <Label>Full Name*</Label>
        <Input className='mb-4' name="name" type='text' onChange={e => setForm({...form,['name']: e.target.value})} required/>

        <Label>Email*</Label>
        <Input className='mb-6' name="email" type='email' onChange={e => setForm({...form,['email']: e.target.value})} required/>
