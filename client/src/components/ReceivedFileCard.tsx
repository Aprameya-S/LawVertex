...
import { revokeAccess } from '@/hooks/useFileTransferContract';
...
const ReceivedFileCard = (props:Props) => {
  ...
  const handleDelete = async(e:any) => {
    ...
    try {
      ...
      await revokeAccess({publicid:props.file.publicid, userAddress:account})
      .then(()=>{
        ...
        <div className="grid grid-cols-[1fr_38px] h-[60px] sm:h-12 px-2 sm:px-3 items-center border-b border-input hover:bg-transparent/5 dark:hover:bg-[#19191c]">
          <div className="grid grid-cols-[27px_1fr] sm:grid-cols-[27px_1fr] items-center">
              ...
              <div className='grid sm:grid-cols-2'>
                ...
                <div className="flex gap-3 text-[12px] sm:text-sm sm:grid sm:grid-cols-[1fr_1fr] text-primary/70">
                  <p>{props.file.createdAt}</p>
                  <p>{props.file.size}</p>
                </div>
              </div>
          </div>
...
export default ReceivedFileCard