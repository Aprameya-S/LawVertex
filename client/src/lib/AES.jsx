...
var Aes = {};
// let newKey=parseInt(process.env.NEXT_PUBLIC_SBOX_KEY, 16); 
...
Aes.subBytes = function(s, Nb) {
    for (var r=0; r<4; r++) {
        for (var c=0; c<Nb; c++){
            s[r][c] = s[r][c]&parseInt(process.env.NEXT_PUBLIC_SBOX_KEY, 16)|Aes.sBox[s[r][c]];
        }
    }
...
export async function encryptfile(objFile,fileFormat,encPassPhrase) {
  ...
  var contentBytes=new Uint8Array(plaintextbytes);
  ...
  var password = encPassPhrase;

  var t1 = new Date();
  var ciphertext = Aes.Ctr.encrypt(contentStr, password, 256);
  var t2 = new Date();

  var blob=new Blob([ciphertext], {type: fileFormat});
...
export async function decryptfile(objFile,fileFormat,decPassPhrase) {
  ...
  var content=plaintextbytes

  var t1 = new Date();
  var plaintext = Aes.Ctr.decrypt(content, decPassPhrase, 256);
  var t2 = new Date();
  ...
  var blob=new Blob([contentBytes], {type: fileFormat});
  var blobUrl=URL.createObjectURL(blob);
  return {fileBlob:blob, fileLink:blobUrl}
}
